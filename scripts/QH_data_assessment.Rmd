...
_The plots in this document are for assessment of stage-discharge measurements and identification of rating curve shifts. For detailed measurement information see []()_
...
ggplotly(ggplot() + 
  geom_line(data = ratingcurve_old, aes(x = Stage_avg, y = Q_model)) + 
  geom_pointrange(data = HQ, aes(x = Stage_avg, y = Q_meas, ymin = Q_meas - Q_abs_unc, ymax = Q_meas + Q_abs_unc, color = Old_New, Label = Event_no), size = 1.5) + 
...
ggplotly(ggplot() + 
  geom_line(data = ratingcurve_old, aes(x = Stage_avg, y = Q_model)) + 
  geom_point(data = HQ_newstorm, aes(x = Stage_avg, y = Q_meas, fill = factor(Event_no), Label = Event_no), shape = 21, size = 2) + 
...
ggplotly(ggplot() + 
  geom_line(data = ratingcurve_old, aes(x = Stage_avg, y = Q_model)) + 
  geom_point(data = HQ, aes(x = Stage_avg, y = Q_meas, fill = factor(WY), Label = Event_no), shape = 21, size = 2) + 
  theme_bw() + xlab("Stage [cm]") + ylab("Discharge [m3/s]") + labs(fill = 'WY'))
...
ggplotly(ggplot() + 
  geom_line(data = ratingcurve_old, aes(x = Stage_avg, y = Q_model)) + 
  geom_point(data = HQ, aes(x = Stage_avg, y = Q_meas, fill = Stage_delta, Label = Event_no), shape = 21, size = 2) + 
