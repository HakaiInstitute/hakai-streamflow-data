...
# =============================================================================
 client <- sn_connect()
#
 pls_raw <- sn_read_values(client, "SSN703US", "5minuteSamples", "PLS_Lvl",
                            start_date = "2014-09-10", end_date = "2018-09-12")
 #sa_raw  <- sn_read_values(client, "SA_WTS703_PT", "5minuteSamples", "SensorDepth_Avg",
                            #start_date = "2021-09-02", end_date = "2025-08-03")
#
...
list<- client$get("api/sn/views/list") 
hecate_rows<-list |> filter(site=="Hecate")

list %>%
  filter(if_any(everything(), ~ str_detect(as.character(.x), regex("hecate", ignore_case = TRUE))))

hecate_probe <- client$get("api/sn/views/Hecate:5minuteSamples?limit=1")
names(hecate_probe)

hecate_rh<- sn_read_values(client, "Hecate", "5minuteSamples", "RH_Avg",
                            start_date = "2014-09-10", end_date = "2018-09-12")
